 ...
 */
static char __buildInfo[] = "FileAttributesPlugin * FileAttributesPlugin.oscog-akg.45 uuid: b0e1cf62-47e3-4a3b-b95c-b639d96c615f " __DATE__ ;
...
static void * structFromObjectsize(sqInt anObject, sqInt structSize);
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"FileAttributesPlugin * FileAttributesPlugin.oscog-akg.45 (i)"
#else
	"FileAttributesPlugin * FileAttributesPlugin.oscog-akg.45 (e)"
#endif
;
static void * sCLPfn;
static void * sCOFfn;
...
primitiveClosedir(void)
{
	...
	faPathPtr = (fapathptr *)structFromObjectsize(dirPointerOop, sizeof(fapathptr));
	if (faPathPtr == 0) {
		return primitiveFailFor(PrimErrBadArgument);
	}
...
{
	popthenPush(1, stringFromCString("2.0.5d06"));
	return 0;
}
 ...
 */
	...
	/* FileAttributesPlugin>>#structFromObject:size: */
static void *
structFromObjectsize(sqInt anObject, sqInt structSize)
{
    void *buffer;

	buffer = 0;
	if (!((stSizeOf(anObject)) == structSize)) {
		primitiveFailFor(PrimErrBadArgument);
		return buffer;
	}
	buffer = malloc(structSize);
	if (buffer == 0) {
		primitiveFailFor(PrimErrNoCMemory);
		return buffer;
	}
	memcpy(buffer, arrayValueOf(anObject), structSize);
	return buffer;
}
